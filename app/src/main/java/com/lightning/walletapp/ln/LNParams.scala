...
object LNParams {
  ...
  def makeLinkingKey(domain: String): PrivateKey = {
    val material: ByteVector = crypto.Mac32.hmac256(key = hashingKey, message = domain)
    derivePrivateKey(chain = hardened(138) +: makeKeyPath(material), parent = master).privateKey
  }
