...
case class HostedCommits(announce: NodeAnnouncement, lastCrossSignedState: LastCrossSignedState, futureUpdates: Vector[LNDirectionalMessage],
  ...
  lazy val withoutUpdatesExceptLocalAdd = me.modify(_.futureUpdates) setTo {
    // Local fail/fulfill can be dropped and regenerated by issuing CMDHTLCProcess
    // but local add message can not be recreated so needs to be retained and resent
    futureUpdates collect { case localAdd @ Left(add: UpdateAddHtlc) => localAdd }
  }

  lazy val nextLocalUnsignedLCSS = {
    ...
    LastCrossSignedState(lastCrossSignedState.refundScriptPubKey, lastCrossSignedState.initHostedChannel,
      broadcaster.currentBlockDay, nextLocalSpec.toLocalMsat, nextLocalSpec.toRemoteMsat, nextTotalLocal, nextTotalRemote,
  ...
  def findFuture(remoteLCSS: LastCrossSignedState) = for {
    previousUpdatesPointIndex <- futureUpdates.indices.drop(1)
    previousUpdates = futureUpdates.take(previousUpdatesPointIndex)
    commitsForPreviousUpdates = me.copy(futureUpdates = previousUpdates)
    if commitsForPreviousUpdates.nextLocalUnsignedLCSS.isEven(remoteLCSS)
  } yield commitsForPreviousUpdates
